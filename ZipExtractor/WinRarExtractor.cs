...
public static class WinRarExtractor
{
    ...
    public static void ExtractRecursively(string archivePath, string rootPath, string?[] passwords)
    {
        ...
        var path = finalResults is [FileInfo]
                   && archiveFile.Directory is { } directory
                   && !directory.FullName.EqualsFileName(rootPath)
            ? directory.Name
            : Path.GetFileNameWithoutExtension(archiveFile.Name);

        // 只有一个文件结果时，使用压缩包所在目录名作为目标文件夹名
        var destPath = Path.Combine(CompletePath, path);
    ...
    private static void MoveArchivesToError(IReadOnlyList<FileInfo> archiveFiles, string? commonPrefix)
    {
        ...
        {
            var dest = FileSystemHelper.GetUniquePath(Path.Combine(ErrorPath, onlyOne.Name));
            if (FileSystemHelper.TryMoveEntry(onlyOne, dest))
                Console.WriteLine("已移动失败压缩包到：" + dest);
            else
                Console.WriteLine($"移动失败压缩包到 {nameof(ErrorPath)} 失败：{onlyOne.FullName} -> {dest}");
            return;
        }

        var destDir = FileSystemHelper.GetUniquePath(Path.Combine(ErrorPath, commonPrefix!));
    ...
    private static void MoveResultsToComplete(IReadOnlyCollection<FileSystemInfo> resultDirs, string destPath)
    {
        foreach (var resultDir in resultDirs)
        {
            var dest = FileSystemHelper.GetUniquePath(Path.Combine(destPath, resultDir.Name));
